...
const SYMBOL_TO_CRYPTOCOMPARE: { [key: string]: string | null } = {
  ...
  'ETH/USD': 'ETH',
  'SOL/USD': 'SOL',
  'PUMP/USD': null, // Not available on CryptoCompare (Solana token)
};

// Map trading symbols to Birdeye Solana token addresses (primary for Solana tokens)
const SYMBOL_TO_BIRDEYE: { [key: string]: string | null } = {
  'BTC/USD': null, // Not a Solana token
  'ETH/USD': null, // Not a Solana token
  'SOL/USD': 'So11111111111111111111111111111111111111112', // Wrapped SOL
  'PUMP/USD': 'pumpCmXqMfrsAkQ5r49WcJnRayYRqmXz6ae8H7H9Dfn', // Official PUMP token on Solana
};
...
export async function registerRoutes(app: Express): Promise<Server> {
  // Proxy endpoint for crypto price data (uses CryptoCompare for major coins, Birdeye for Solana tokens, fallback to CoinGecko)
  app.get('/api/crypto/price', async (req, res) => {
    try {
      const symbol = req.query.symbol as string || 'SOL/USD';
      ...
      // Try Birdeye for Solana tokens (PUMP, SOL as fallback)
  ...
  app.get('/api/crypto/chart', async (req, res) => {
    try {
      ...
      // Try Birdeye for Solana tokens if available (for current price to generate synthetic chart)
      const birdeyeAddress = SYMBOL_TO_BIRDEYE[symbol];
      if (birdeyeAddress && process.env.BIRDEYE_API_KEY) {
        try {
  ...
  // Solana RPC Proxy - Required because public RPC blocks browser requests
  app.post('/api/solana-rpc-proxy', async (req, res) => {
    try {
      console.log('[Solana RPC Proxy] Received request:', req.body);
      ...
      const data = await response.json();
      console.log('[Solana RPC Proxy] Sending response:', data);
  ...
  // Jupiter Price API Proxy - Required because Jupiter blocks browser CORS requests
  app.get('/api/jupiter-price-proxy', async (req, res) => {
    try {
      ...
      const response = await fetch(
        `https://price.jup.ag/v6/price?ids=${encodeURIComponent(ids)}`
      );
      ...
      res.status(500).json({ error: 'Failed to fetch prices from Jupiter' });
    }
  });

  // Wallet Profile API Routes
  // NOTE: Production requires Solana signature verification to prove wallet ownership
